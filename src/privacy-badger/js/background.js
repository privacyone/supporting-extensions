...
Badger.prototype = {
   ...
   */
  defaultSettings: {
    checkForDNTPolicy: true, //Breeze
    disabledSites: [],
    disableGoogleNavErrorService: true,
    disableHyperlinkAuditing: true,
    hideBlockedElements: true,
    learnInIncognito: false,
    learnLocally: true, //Breeze
    migrationLevel: 0,
    preventWebRTCIPLeak: true, //Breeze
    seenComic: true, //Breeze
    sendDNTSignal: true,
    showCounter: true,
    showIntroPage: false, //Breeze
  ...
  logThirdPartyOriginOnTab: function (tab_id, fqdn, action) {
    ...
    //Breeze
    if(action == "block"){
      chrome.runtime.sendMessage("eabnfhopkgkfldmbhljheahllnecbbij", 
      {
        what: "badger_domain_block", 
        url: fqdn
      })
    }
